...
public class EditTableImpl<R> {
	...
	public void update(R row) {
		...
		for (int x = 0; x < columns.size(); x++) {
			...
			for (int y = 0; y < list.size(); y++) {
				...
				column = this.wrapper.getConcretColumn(column, row, context);
	...
	public void updateFromGUI(R row) {
		...
		for (int x = 0; x < columns.size(); x++) {
			...
			for (int y = 0; y < list.size(); y++) {
				EditColumn<R> column = list.get(y);
				column = this.wrapper.getConcretColumn(column, row, null);
				IsWidget widget = table.getWidget(index, y);
	...
	public void updateRows(List<R> rows) {
		for (R row : rows) {
			update(row);
		}
	}
	...
	protected Element getRowElement(int row) {
		return this.rowFormatter.getElement(row);
	}
	...
	public void focusConsultantTable()
	{
		NativeEvent event = Document.get().createClickEvent(1, 0, 0, 0, 0, false,
				   false, false, false);
		DomEvent.fireNativeEvent(event,getFirstChild(table.getWidget(1, 0)));
		DomEvent.fireNativeEvent(event,table.getWidget(1, 1));
//		Scheduler.get().scheduleDeferred(new ScheduledCommand() {
//		    @Override
//		    public void execute() {
//		    	// To select the value in the fist column of the table
////				getFirstChild(table.getWidget(1, 0)).getElement().focus();
//			
//		    }
//		});
