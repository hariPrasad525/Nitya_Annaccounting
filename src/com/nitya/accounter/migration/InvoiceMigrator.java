...
public class InvoiceMigrator extends TransactionMigrator<Invoice> {
	...
	public JSONObject migrate(Invoice obj, MigratorContext context) throws JSONException {
		...
		external.put("com.nitya.accounter.shared.customer.Invoice", jsonObject);
		...
		if (!estimates.isEmpty()) {
			...
			salesOrders.put("com.nitya.accounter.shared.customer.SalesOrder", array);
		...
		if (!estimates.isEmpty()) {
			...
			credits.put("com.nitya.accounter.shared.customer.Credit", array);
			for (Estimate estimate : estimates) {
		...
		if (!estimates.isEmpty()) {
			...
			salesQuotes.put("com.nitya.accounter.shared.customer.SalesQuotation", array);
			for (Estimate estimate : estimates) {
		...
		if (contact != null && !contact.getName().isEmpty()) {
			{
				...
				outter1.put("com.nitya.accounter.shared.common.Contact", inter1);
