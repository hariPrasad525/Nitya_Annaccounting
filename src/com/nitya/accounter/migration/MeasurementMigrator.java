...
public class MeasurementMigrator implements IMigrator<Measurement> {
	...
	public JSONObject migrate(Measurement obj, MigratorContext context) throws JSONException {
		JSONObject measurement = new JSONObject();
		{
			...
			String key = "com.nitya.accounter.shared.inventory.Unit";
			List<Long> list = childrenMap.get(key);
			if (list == null) {
				list = new ArrayList<Long>();
				childrenMap.put(key, list);
			}
			measurement.put("com.nitya.accounter.shared.inventory.Measurement", jsonObject);
			...
			unitsObj.put("com.nitya.accounter.shared.inventory.Unit", units);
			for (Unit unit : obj.getUnits()) {
				...
				if (unit.isDefault()) {
					JSONObject defaultUnitObject = new JSONObject();
					{
						...
						defaultUnitObject.put("com.nitya.accounter.shared.inventory.Unit", json);
					}
