...
public class TransactionMigrator<T extends Transaction> implements IMigrator<T> {
	...
	public JSONObject migrate(T obj, MigratorContext context)
		...
		if (obj instanceof SalesOrder) {
			key = "com.nitya.accounter.shared.customer.SalesOrderItem";
			context.putChilderName("SalesOrder", "transactionItems");
		}
		if (obj instanceof PurchaseOrder) {
			key = "com.nitya.accounter.shared.vendor.PurchaseOrderItem";
		...
		if (obj instanceof Estimate) {
			...
			if (estimateType == Estimate.QUOTES
				...
				key = "com.nitya.accounter.shared.customer.SalesQuotationItem";
				context.putChilderName("SalesQuotation", "transactionItems");
			} else if (estimateType == Estimate.CREDITS) {
				key = "com.nitya.accounter.shared.customer.TransactionCreditItem";
		...
		if (accounterClass != null) {
			...
			outter.put("com.nitya.accounter.shared.common.AccountClass",
					inter);
		...
		if (location != null) {
			JSONObject inter = new JSONObject();
			JSONObject outter = new JSONObject();
			outter.put("com.nitya.accounter.shared.common.Location", inter);
			...
			for (TransactionItem transactionItem : transactionItems) {
				...
				if (transactionItem.getType() == TransactionItem.TYPE_ACCOUNT) {
					...
					Account account = transactionItem.getAccount();
					{
						...
						JSONObject outter = new JSONObject();
						outter.put(
								"com.nitya.accounter.shared.common.Account",
								inter);
						inter.put("@_oid",
					...
					}
					{
						...
						if (unit != null) {
							...
							JSONObject outter = new JSONObject();
							outter.put(
									"com.nitya.accounter.shared.inventory.Unit",
									inter);
				...
				if (classCategory != null) {
					...
					JSONObject outter = new JSONObject();
					outter.put(
							"com.nitya.accounter.shared.common.AccountClass",
							inter);
				...
				if (taxPerDetailLine && itemTaxCode != null) {
					JSONObject inter = new JSONObject();
					JSONObject outter = new JSONObject();
					outter.put("com.nitya.accounter.shared.tax.TAXCode",
							inter);
					inter.put("@_oid",
				...
				if (txn != null) {
					...
					JSONObject outter = new JSONObject();
					outter.put(
							"com.nitya.accounter.shared.common.TaxableTransaction",
							inter);
				...
				if (wareHouse != null) {
					...
					JSONObject outter = new JSONObject();
					outter.put(
							"com.nitya.accounter.shared.inventory.Warehouse",
							inter);
					inter.put("@_oid",
				...
				if (obj instanceof EnterBill) {
					...
					if (billableJob != null) {
						...
						JSONObject outter = new JSONObject();
						outter.put(
								"com.nitya.accounter.shared.customer.Project",
								inter);
						String localId = context.getLocalIdProvider()
					...
					if (customer != null) {
						...
						JSONObject outter = new JSONObject();
						outter.put(
								"com.nitya.accounter.shared.customer.Customer",
								inter);
						String localId = context.getLocalIdProvider()
				...
				if ((enterBill != null || cashPurchase != null)
					...
					createJSON(
							context,
							"com.nitya.accounter.shared.vendor.PurchaseOrderItem",
	...
	private String getItemType(Item item) {
		...
		case Item.TYPE_SERVICE:
			return "com.nitya.accounter.shared.common.ServiceItem";
		case Item.TYPE_NON_INVENTORY_PART:
			return "com.nitya.accounter.shared.common.ProductItem";
		case Item.TYPE_INVENTORY_PART:
			return "com.nitya.accounter.shared.inventory.InventoryItem";
		case Item.TYPE_INVENTORY_ASSEMBLY:
			return "com.nitya.accounter.shared.inventory.InventoryAssembly";
		}
		return null;
	}
	...
	private String getType(T obj) {
		...
		case Transaction.TYPE_CASH_SALES:
			return "com.nitya.accounter.shared.customer.CashSaleItem";
		case Transaction.TYPE_CASH_PURCHASE:
			return "com.nitya.accounter.shared.vendor.CashPurchaseItem";
		case Transaction.TYPE_CUSTOMER_CREDIT_MEMO:
			return "com.nitya.accounter.shared.customer.CreditMemoItem";
		case Transaction.TYPE_ENTER_BILL:
			return "com.nitya.accounter.shared.vendor.EnterBillItem";
		case Transaction.TYPE_ESTIMATE:
			return getEstimateType(obj);
		case Transaction.TYPE_INVOICE:
			return "com.nitya.accounter.shared.customer.InvoiceItem";
		case Transaction.TYPE_PAY_BILL:
			return "com.nitya.accounter.shared.vendor.PayBillItem";
		case Transaction.TYPE_RECEIVE_PAYMENT:
			return "com.nitya.accounter.shared.customer.ReceivePaymentItem";
		case Transaction.TYPE_VENDOR_CREDIT_MEMO:
			return "com.nitya.accounter.shared.vendor.DebitNoteItem";
		case Transaction.TYPE_WRITE_CHECK:
			return "com.nitya.accounter.shared.common.WriteCheckAccountItem";
		case Transaction.TYPE_JOURNAL_ENTRY:
			return "com.nitya.accounter.shared.common.JournalEntryItem";
		case Transaction.TYPE_PAY_TAX:
			return "com.nitya.accounter.shared.tax.PayTAXItem";
		case Transaction.TYPE_EXPENSE:
			return "";
		case Transaction.TYPE_PAY_EXPENSE:
			return "";
		case Transaction.TYPE_TAX_RETURN:
			return "com.nitya.accounter.shared.tax.FileTaxItem";
		case Transaction.TYPE_PURCHASE_ORDER:
			return "com.nitya.accounter.shared.vendor.PurchaseOrderItem";
		case Transaction.TYPE_ITEM_RECEIPT:
			return "";
		case Transaction.TYPE_ADJUST_VAT_RETURN:
			return "";
		case Transaction.TYPE_CASH_EXPENSE:
			return "com.nitya.accounter.shared.vendor.PurchaseExpenseItem";
		case Transaction.TYPE_CREDIT_CARD_EXPENSE:
			return "com.nitya.accounter.shared.vendor.PurchaseExpenseItem";
		case Transaction.TYPE_EMPLOYEE_EXPENSE:
			return "";
		case Transaction.TYPE_RECEIVE_TAX:
			return "com.nitya.accounter.shared.tax.TAXRefundItem";
		case Transaction.TYPE_TDS_CHALLAN:
			return "com.nitya.accounter.shared.tax.TdsChallanItem";
		case Transaction.TYPE_MAKE_DEPOSIT:
			return "com.nitya.accounter.shared.bank.MakeDepositItem";
		case Transaction.TYPE_STOCK_ADJUSTMENT:
			return "com.nitya.accounter.shared.inventory.StockAdjustmentItem";
		case Transaction.TYPE_BUILD_ASSEMBLY:
			return "com.nitya.accounter.shared.inventory.BuildAssemblyItem";
		case Transaction.TYPE_SALES_ORDER:
			return "com.nitya.accounter.shared.customer.SalesOrderItem";
		case Transaction.TYPE_STOCK_TRANSFER:
			return "com.nitya.accounter.shared.inventory.WarehouseTransferItem";
		}
		return "com.nitya.accounter.shared.common.TransactionItem";
	}
	...
	private String getEstimateType(Transaction transaction) {
		...
		case Estimate.BILLABLEEXAPENSES:
		case Estimate.CHARGES:
			return "com.nitya.accounter.shared.customer.SalesQuotationItem";
		case Estimate.CREDITS:
			return "com.nitya.accounter.shared.customer.TransactionCreditItem";
