...
import com.nitya.accounter.core.TransactionReceivePayment;
...
public class ReceivePaymentMigrator extends TransactionMigrator<ReceivePayment> {
	...
	public JSONObject migrate(ReceivePayment obj, MigratorContext context) throws JSONException {
		...
		external.put("com.nitya.accounter.shared.customer.ReceivePayment", receivePayment);
		...
		List<TransactionReceivePayment> transactionReceivePayment = obj.getTransactionReceivePayment();
		{
			...
			items.put("com.nitya.accounter.shared.customer.ReceivePaymentItem", receivePaymentItems);
			for (TransactionReceivePayment item : transactionReceivePayment) {
				...
				}
				{
					...
					currencyJson.put("com.nitya.accounter.shared.customer.Invoice", json);
				...
				if (payment != null) {
					JSONObject inter = new JSONObject();
					JSONObject outter = new JSONObject();
					outter.put("com.nitya.accounter.shared.customer.ReceivePayment", inter);
					inter.put("@id", _lid);
					inter.put("@_lid", _lid);
				...
				// Apply Credits
				{
					if (!item.getTransactionCreditsAndPayments().isEmpty()) {
						...
						acOut.put("com.nitya.accounter.shared.customer.ApplyCredit", applyCredit);
						JSONArray applyCreditItems = new JSONArray();
						JSONObject acItems = new JSONObject();
						acItems.put("com.nitya.accounter.shared.customer.ApplyCreditItem", applyCreditItems);
						for (TransactionCreditsAndPayments ac : item.getTransactionCreditsAndPayments()) {
							...
							applyCreditItem.put("@_lid", localId);
							{
								JSONObject inter = new JSONObject();
								JSONObject outter = new JSONObject();
								outter.put("com.nitya.accounter.shared.customer.ApplyCredit", inter);
							...
							}
							{
								JSONObject inter = new JSONObject();
								JSONObject outter = new JSONObject();
								outter.put("com.nitya.accounter.shared.customer.Credit", inter);
